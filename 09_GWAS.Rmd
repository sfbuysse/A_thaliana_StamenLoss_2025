...
# want to count the number of rows that are not NAs, if everything else is an NA, this should equal 3 for the first 3 intro rows
# so this is only keeping rows where it equals three, which is the exact opposite of what I want to do
# so instead is count NAs, and  keep rows with fewer than 24 NAs
top_hits_fdr_noelev <- top_hits_fdr_noelev[rowSums(is.na(top_hits_fdr_noelev)) < 24 , ] 
...
hits_fdr_chr1 <- top_hits_fdr_noelev[(top_hits_fdr_noelev$chr == 1), c("chr", "rs", "ps", grep("p_", colnames(top_hits_fdr_noelev), value = TRUE))]
...
for (i in c(1:length(hits_fdr_chr1$ps))){
  ...
  # the loop below this doesn't count if a single snp is in two GWAS, so maybe I do need to change this code.
  if (length(tmp_df[,1]) > 2) {
    print(hits_fdr_chr1$rs[i])
    print(tmp_df)}
  print("End")
}
# now that my sample size is larger, this output is kinda ridiculous to go through by hand. So I want to add something that will make sure it is at least two different types of gwas I think
