...
nocent_bin <- read.delim(file = paste0("C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/GemmaOutput_2022/", files[1], ".assoc.txt"))
...
top_hits <- read.csv("C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/GemmaOutput_2022/Top_hits_wald_Jan2023.csv")
...
# what I ideally want, is to actually only print the dataframe if it is more than two rows?
...
top_hits_fdr_noelev <- top_hits_fdr_all[, grep("elev", colnames(top_hits_fdr_all), value = TRUE, invert = TRUE)]
...
top_hits_fdr_noelev_nocent <- top_hits_fdr_all[, grep("all", colnames(top_hits_fdr_noelev), value = TRUE, invert = TRUE)]

# so count NAs, and  keep rows with fewer than 12 NAs (3 columns should have values for all rows)
top_hits_fdr_noelev_nocent <- top_hits_fdr_noelev_nocent[rowSums(is.na(top_hits_fdr_noelev_nocent)) < 12 , ] 
# sample size went way down!

#### chr 1 ####

# only keep the p value columns
hits_fdr_chr1 <- top_hits_fdr_noelev_nocent[(top_hits_fdr_noelev_nocent$chr == 1), c("chr", "rs", "ps", grep("p_", colnames(top_hits_fdr_noelev_nocent), value = TRUE))]
...
for (i in c(1:length(hits_fdr_chr1$ps))){
  ...
  if (length(tmp_df[,1]) > 2) {
    chr1_overlap <- append(chr1_overlap, list(tmp_df))}
  #print("End")
}
# now that my sample size is larger, this output is kinda ridiculous to go through by hand. So I want to add something that will make sure it is at least two different types of gwas I think (insted of a bunch of hits in binary that are overlapping which I do see now)
# the bigger concern might actually be trying to not have the duplicate outputs

# output makes more sense when ran in the console

# successfully make an output that is a list of dataframes! This still has 160 entries, so how to go through that with code?
length(unique(chr1_overlap)) # only 74 are unique, that seems promising; it is just the first 74 though and that seems odd
chr1_overlap[[75]] # I did manually find this in the unique output. manually checked: 75, 100, 104, 118, 145, 87 and found duplicates for all of them!
...
# manually scrolling through, all of the overlap just with nocent is only within a single gwas type... what changed?

# maybe try it on all instead of NC?

```

I stopped working in the chunk above this note on 4/26/2023

```{r}
## not updated to use top SNPs based on p value.
# what I might wnat to do is figure out the LD decay in my sample and use that to determine window size?
...
- raw and subset overlap most on chromsome 1, asin and raw overlap most on chromosome 5
