...
eigenValues_pruned <- read.delim("C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/R_script/StamenLossPipeline/data/plinkPCA_pruned.eigenval", sep = " ", header = F)
eigenVectors_pruned <- read.delim("C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/R_script/StamenLossPipeline/data/plinkPCA_pruned.eigenvec", sep = " ", header = F)
...
#eigenValues_noCent2 <- read.delim("C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/R_script/StamenLossPipeline/data/plinkPCA_NoCent_2.eigenval", sep = " ", header = F) #this onen also exists?
#identical(eigenValues_noCent, eigenValues_noCent2)
# oh these are very different... going with the one that was originally referenced... found it ! the commented out one does not have plink filtering.
...
ggsave(filename = "plink_PC1PC2_PlinkFiltering_topo_fill.png", path = "C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/Figures/ManuscriptFigs", height = 3, width = 3, device = "png", dpi = 700)
...
annotate_figure(plot_s4a,
                fig.lab = "A", fig.lab.face = "bold")
ggsave(filename = "plink_PC3PC4_PlinkFiltering_topo_fill.png", path = "C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/Figures/ManuscriptFigs/supp", height = 3, width = 3, device = "png", dpi = 700)
...
ggsave(filename = "plink_PC1PC2_PlinkFiltering_noBOS_topo_fill.png", path = "C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/Figures/ManuscriptFigs", height = 3, width = 3, device = "png", dpi = 700)
...
annotate_figure(plot_s4b,
                fig.lab = "B", fig.lab.face = "bold")
ggsave(filename = "plink_PC3PC4_PlinkFiltering_noBOS_topo_fill.png", path = "C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/Figures/ManuscriptFigs/supp", height = 3, width = 3, device = "png", dpi = 700)

```

NoBOS Figure legend

abandoned this code b/c I think I figured out the NOBOS stuff.
```{r, eval = FALSE}
ggplot(eigenVectors_noBOS)+
  geom_point(data=eigenVectors_noBOS, aes(x=rep(1, times = 16), y=rev(c(1:16)), shape = Pop, fill= elev), col = "black", size = 6, stroke = 1.75, show.legend = TRUE)+
  geom_text(aes(x=rep(1, times = 16), y=rev(c(1:16)), label = unique(eigenVectors_noBOS$label)), size = 20/.pt, hjust = -0.25)+
...
annotate_figure(plot_s5a,
                fig.lab = "A", fig.lab.face = "bold")
ggsave(filename = "plink_PC1PC2_PlinkFiltering_noCent_topo_fill.png", path = "C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/Figures/ManuscriptFigs/supp", height = 3, width = 3, device = "png", dpi = 700)
...
annotate_figure(plot_s5b,
                fig.lab = "B", fig.lab.face = "bold")
ggsave(filename = "plink_PC3PC4_PlinkFiltering_noCent_topo_fill.png", path = "C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/Figures/ManuscriptFigs/supp", height = 3, width = 3, device = "png", dpi = 700)
...
ggsave(filename = "plink_PC1PC2_PlinkFiltering_noCent_noBOS_topo_fill.png", path = "C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/Figures/ManuscriptFigs/supp", height = 3, width = 3, device = "png", dpi = 700)
...
ggsave(filename = "plink_PC3PC4_PlinkFiltering_noCent_noBOS_topo_fill.png", path = "C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/Figures/ManuscriptFigs/supp", height = 3, width = 3, device = "png", dpi = 700)
```
manually moved graphs to local computer either with filezilla or just within mobaxterm. But then realized I needed to use local machine to make high enough quality figures.
...
# then Jeff asked for a multiple regression of each PC on all three variables. I can easily do this, but I'm not sure about the application to be honest because it still isn't how you test for IBD because I am not using the geographic distance between each population.
...
Old code that has psuedereplication because multiple PC values per population and only one elevation per population.
...
plink -bfile /mnt/scratch/buysseso/GVCF/allChrs_SNPs_NoCent --het --out /mnt/scratch/buysseso/PCA/indv_plink_het_noCent
```
...
NOTE! het_dat is saved at the end of this code, so you can start by reading that in.
```{r}
library(ggplot2)
...
# manually made this one comma separated becuase it was easier.
dat_nc <- read.delim("/mnt/scratch/buysseso/PCA/indv_plink_het_noCent.het", header = TRUE, sep = ",")
str(dat_nc)
...
colnames(dat_nc)
dat_nc$O.HET <- dat_nc$N.NM. - dat_nc$O.HOM.
dat_nc$freq_het <- dat_nc$O.HET/dat_nc$N.NM.

plot(dat$freq_het)
# outlier remains, I think that is a good sign honestly.

# let's make both dataframes ordered the same way
dat <- dat[order(dat$IID), ]
dat_nc <- dat_nc[order(dat_nc$IID), ]
identical(dat$IID, dat_nc$IID)
# true, yay

# let's put the columns of most interest together
het_dat <- data.frame(dat$IID, dat$freq_het, dat_nc$IID, dat_nc$freq_het)

cor(het_dat$dat.freq_het, het_dat$dat_nc.freq_het, method = "pearson")
# 0.3313797 well that seems low.
plot(dat$freq_het, dat_nc$freq_het)
# hmm, well they are different outliers which is not a good sign I don't think...
# besides the outliers they seem decently highly correlated?

# can I do a correlation without the outliers?
tmp <-  het_dat[!(het_dat$dat.IID %in% c("MUR16", "Ale-4")), ]
cor(tmp$dat.freq_het, tmp$dat_nc.freq_het, method = "pearson")
#  0.9792351
# yeah that is more what I was expecting. not entirely sure how to deal with this. IS Ale-4 different on the pCA?
# ALE-4 does have a point away from the rest on both PC1/PC2 and PC3/PC4. huh, well that is the new outlier to investigate I guess.

# hmm wait but Mur16 is actually kinda similar in the two datasets.
tmp2 <-  het_dat[!(het_dat$dat.IID %in% c("Ale-4")), ]
cor(tmp2$dat.freq_het, tmp2$dat_nc.freq_het, method = "pearson")
# 0.980302 still good, so the change is just in Ale-4
plot(tmp2$dat.freq_het, tmp2$dat_nc.freq_het)

# let's make a pop column
het_dat$pop <- toupper(substr(het_dat$dat.IID, 1, 3))

# histogram
ggplot(data = het_dat)+
  geom_histogram(aes(dat.freq_het), bins = 15)+
  theme_classic()

ggplot(data = het_dat)+
  geom_histogram(aes(dat_nc.freq_het), bins = 35)+
  theme_classic()
ggplot(data = tmp)+
  geom_histogram(aes(dat_nc.freq_het), bins = 15)+
  theme_classic()
ggplot(data = tmp2)+
  geom_histogram(aes(dat_nc.freq_het), bins = 15)+
  theme_classic()
...
het_dat[het_dat$pop == "ARB", "pop"] <- "ARU"
het_dat[het_dat$pop == "SPE", "pop"] <- "SAL"
...
# merge 
het_dat$elev <- metadata$Elev_m[match(het_dat$pop, metadata$PopCode)]
# then match with row names to add metadata column
# do this for lat and long now too
het_dat$lat <- metadata$Lat_DecDeg[match(het_dat$pop, metadata$PopCode)]
het_dat$lon <- metadata$Lon_DecDeg[match(het_dat$pop, metadata$PopCode)]

# order by elevation
het_dat <- het_dat[order(het_dat$elev),]
het_dat$label <- paste0(het_dat$pop, " - ", het_dat$elev, "m")
het_dat$pop <- factor(het_dat$pop, levels = unique(het_dat$pop[order(het_dat$elev)]))
# need to force an x value for the plot to work
het_dat$x_val <- c(1:61)

# if you skipped stuff, load in the data here
load("C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/R_script/StamenLossPipeline/data/heterozygosity_comparison.ROBJ")

### with centromere ###
# new plot
ggplot(data = het_dat)+
  geom_point(aes(x = x_val, y = dat.freq_het, fill = elev, shape = pop), col = "black", size = 3, stroke = 1.25, show.legend = FALSE)+
  geom_text(aes(x = x_val, y = dat.freq_het, label = dat.IID), nudge_x = 0, nudge_y = -0.005, col = "black")+
  labs(x = " ", y = "Observed Heterozygosity (frequency)")+
  scale_shape_manual(name = "Population", labels = unique(het_dat$label), values = rep(c(22, 21, 24, 23, 25), times = 4))+
  scale_fill_gradientn(name = "Elevation", colours = topo.colors(16))+
  theme_classic()+
  theme(
    legend.title = element_text(color = "black", size = 20),
    ...
    axis.title = element_text(color = "black", size = 20),
    axis.text = element_text(color = "black", size = 20),
    legend.spacing.y = unit(0.03, "cm")) # was 0.001
ggsave(filename = "plink_het_PlinkFiltering_labels.png", path = "C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/Figures/ManuscriptFigs", height = 7, width = 9, dpi = 500)
# old plot
ggplot(data = het_dat)+
  geom_point(aes(x = x_val, y = dat.freq_het, col = pop, shape = pop), size = 3, stroke = 1.25, show.legend = TRUE)+
  geom_text(aes(x = x_val, y = dat.freq_het, label = dat.IID), nudge_x = 0, nudge_y = -0.005, col = "black")+
  labs(x = " ", y = "Observed Heterozygosity (frequency)")+
  scale_shape_manual(name = "Population", labels = unique(het_dat$label), values = rep(c(0, 1, 2, 5), times = 4))+
  scale_color_manual(name = "Population", labels = unique(het_dat$label), values = rep(c("red", "orange", "green", "blue"), times = c(4,4,4,4)))+
  theme_classic()+
  theme(
    legend.title = element_text(color = "black", size = 12),
    legend.text = element_text(color = "black", size = 10),
...
# new plot
ggplot(data = het_dat)+
  geom_point(aes(x = x_val, y = dat_nc.freq_het, fill = elev, shape = pop), col = "black", size = 3, stroke = 1.25, show.legend = FALSE)+
  geom_text(aes(x = x_val, y = dat_nc.freq_het, label = dat_nc.IID), nudge_x = 0, nudge_y = -0.02, col = "black")+
  labs(x = " ", y = "Observed Heterozygosity (frequency)")+
  scale_shape_manual(name = "Population", labels = unique(het_dat$label), values = rep(c(22, 21, 24, 23, 25), times = 4))+
  scale_fill_gradientn(name = "Elevation", colours = topo.colors(16))+
  theme_classic()+
  theme(
    ...
    axis.text = element_text(color = "black", size = 20),
    legend.spacing.y = unit(0.03, "cm")) # was 0.001
ggsave(filename = "plink_het_PlinkFiltering_noCent_labels.png", path = "C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/Figures/ManuscriptFigs", height = 7, width = 9, dpi = 500)


# save dat with the updates as an R object!
#save(dat, file = "/mnt/scratch/buysseso/PCA/heterozygosity_df.ROBJ")

# save het_dat as an R object
save(het_dat, file = "/mnt/scratch/buysseso/PCA/heterozygosity_comparison.ROBJ")
```
