...
# now add block into that fancy model and try again
# wait maybe not. block isn't in the sequenced dataframe??? not sure why not. Did I make that subset?
...
subset <- read.delim("C:/Users/Sophie/Michigan State University/Conner, Jeffrey - SophieAnalyses/FamFiles/nonzero.fam", header=FALSE)
...
Updating on 11/13/2024 to edit the qqplot font sizes since I finally figured out how to do that. Only made a good plot for the arcsin that goes in the main text cause it did take some cropping and manual editing after the fact.
...
gwas_plots <- function(type, identifier, sig.level = 0.05){
  ...
  png(qqplot_name, width = 2.2, height = 2.2, units = 'in', res = 700)
...
# and plot. should make 3 plots.
  ...
  # need to figure out what the suggested lines are drawn at in the qqman package
...
Chr 1 overlap analysis but includes elevation this time - done 1/25/2024
...
chr1_overlap_nc <- id_snps(hits_fdr_chr1)
#1823 elements
length(unique(chr1_overlap_nc)) # 1184 unique
...
# fancy function I wrote
chr1_overlap_all <- id_snps(hits_fdr_chr1_a) # 1676 elements
length(unique(chr1_overlap_all)) # 1093 are unique
# fancy function 2
OverlapHits_chr1_all <- gwas_overlap(chr1_overlap_all, all_p_cols)
# list of 14
```
...
chr2_overlap_nc <- id_snps(hits_fdr_chr2)
#4301 elements
length(unique(chr2_overlap_nc)) # 2580 unique
...
# fancy function I wrote
chr2_overlap_all <- id_snps(hits_fdr_chr2_a) # 3124 elements
length(unique(chr2_overlap_all)) # 1933 are unique
# fancy function 2
OverlapHits_chr2_all <- gwas_overlap(chr2_overlap_all, all_p_cols)
# list of 44
```
...
chr3_overlap_nc <- id_snps(hits_fdr_chr3)
#2689 elements
length(unique(chr3_overlap_nc)) # 1647 unique
...
# fancy function I wrote
chr3_overlap_all <- id_snps(hits_fdr_chr3_a) # 2349 elements
length(unique(chr3_overlap_all)) # 1453 are unique
# fancy function 2
OverlapHits_chr3_all <- gwas_overlap(chr3_overlap_all, all_p_cols)
# list of 94
```
...
chr4_overlap_nc <- id_snps(hits_fdr_chr4)
#1202 elements
length(unique(chr4_overlap_nc)) # 761 unique
...
# fancy function I wrote
chr4_overlap_all <- id_snps(hits_fdr_chr4_a) # 1344 elements
length(unique(chr4_overlap_all)) # 938 are unique
# fancy function 2
OverlapHits_chr4_all <- gwas_overlap(chr4_overlap_all, all_p_cols)
# list of 10
```
...
chr5_overlap_nc <- id_snps(hits_fdr_chr5)
#3437 elements
length(unique(chr5_overlap_nc)) # 1950 unique
...
# fancy function I wrote
chr5_overlap_all <- id_snps(hits_fdr_chr5_a) # 3569 elements
length(unique(chr5_overlap_all)) # 2097 are unique
# fancy function 2
OverlapHits_chr5_all <- gwas_overlap(chr5_overlap_all, all_p_cols)
# list of 77
...
#### old code: highlight certain SNPs in the GWAS figures.

New code is in a separate file completely.
