...
---
Need document with less strict filtering parameters... why not just use the allsites one? try that first to see if it works.
script is called Fst_sub_2022 for the May 2022 version (most recent - on the HPCC - all code also copied into this script for records)

load R/4.1.2 on HPCC to get packages to load but note using R/4.2.2  on both my laptop and my lab PC
...
Was cleaning this file up initially and did remove full file locations in save lines at some points so can't run top to bottom for figure code.
...
# this output is an excel file that cannot be looked at manually.
...
colnames(tmp) <- toupper(colnames(tmp))
colnames(tmp)
# make column names all uppercase and remove dashes so all in the same format. Still identifiable by unique plant here, not by population.
class(tmp)
...
This was done on my local machine. Did some manual renaming of files and changing text files to .csv files
...
## read in the matrix
sub_10k <- read.csv("~/PopGen/fastDivPart_plinkFilter_sub_SNPs_Dec16_2022/pairwiseFst_WC.csv", header = TRUE)
...
save(fst_mat, file = "~/Fst_matrix_Dec2022_10k.ROBJ")

pdf("~/Figures/Fst_heatmap_Dec2022_10k.pdf", height = 9, width = 11)
...
pheatmap(elev_mat, cluster_rows = FALSE, cluster_cols = FALSE, color = colorRampPalette(brewer.pal(n = 9, name =
  "YlOrRd"))(100), border_color = NA, labels_row = row.labs, fontsize_row = 15, fontsize_col = 15, filename = "~/Figures/ManuscriptFigs/supp/fst_heatmap_elevation_Dec2022_10k.png",height = 9, width = 9)

##### Now with when Fst was calculated with population identifiers #####

sub_10k_pop <- read.csv("~/PopGen/fastDivPart_plinkFilter_sub_SNPs_Pop_Dec16_2022/pairwiseFst_WC.csv", header = TRUE)
...
substr(rownames(fst_mat),1,3)[substr(rownames(fst_mat),1,3) == 'SPE'] <- "SAL"
substr(colnames(fst_mat),1,3)[substr(colnames(fst_mat),1,3) == 'SPE'] <- "SAL"
save(fst_mat, file = "~/Fst_matrix_Dec2022_10k_pop.ROBJ")

pdf("~/Figures/Fst_heatmap_Dec2022_10k_pop.pdf", height = 9, width = 11)
heatmap(x = fst_mat, symm = TRUE,
        xlab = "Population",
        ylab = "Population",
        main = "Fst Heatmap", 
...
pheatmap(elev_mat, cluster_rows = FALSE, cluster_cols = FALSE, color = colorRampPalette(brewer.pal(n = 9, name =
  "YlOrRd"))(100), border_color = NA, labels_row = row.labs, fontsize_row = 15, fontsize_col = 15, filename = "~/Figures/fst_heatmap_elevation_Dec2022_10k_pop.png",height = 9, width = 9)
...
Note: code seems to be out of order here but I did the Fst calculation without the centromeres, and then visualized it so this code is reading in the no centromere results.
...
## read in the matrix
sub_10k <- read.csv("~/fastDivPart_plinkFilter_sub_NoCent_SNPs_Jan5_2023/pairwiseFst_WC.csv", header = TRUE)
...
substr(rownames(fst_mat),1,3)[substr(rownames(fst_mat),1,3) == 'SPE'] <- "SAL"
substr(colnames(fst_mat),1,3)[substr(colnames(fst_mat),1,3) == 'SPE'] <- "SAL"
save(fst_mat, file = "~/Fst_matrix_NoCent_Dec2022_10k.ROBJ")

pdf("~/Figures/Fst_heatmap_NoCent_Dec2022_10k.pdf", height = 9, width = 11)
heatmap(x = fst_mat, symm = TRUE,
        xlab = "Population",
        ylab = "Population",
        main = "Fst Heatmap", 
...
pheatmap(elev_mat, cluster_rows = FALSE, cluster_cols = FALSE, color = colorRampPalette(brewer.pal(n = 9, name =
  "YlOrRd"))(100), border_color = NA, labels_row = row.labs, fontsize_row = 15, fontsize_col = 15, filename = "~/Figures/ManuscriptFigs/supp/fst_heatmap_NoCent_elevation_Dec2022_10k.png",height = 9, width = 9)


##### Now with when Fst was calculated with population identifiers #####

sub_10k_pop <- read.csv("~/PopGen/fastDivPart_plinkFilter_sub_NoCent_SNPs_Pop_Jan6_2023/pairwiseFst_WC.csv", header = TRUE)
...
fst_mat <- sub_10k_pop
save(fst_mat, file = "~/Fst_matrix_NoCent_Dec2022_10k_pop.ROBJ")

pdf("~/Figures/Fst_heatmap_NoCent_Dec2022_10k_pop.pdf", height = 9, width = 11)
heatmap(x = fst_mat, symm = TRUE,
        xlab = "Population",
        ylab = "Population",
        main = "Fst Heatmap", 
...
pheatmap(elev_mat, cluster_rows = FALSE, cluster_cols = FALSE, color = colorRampPalette(brewer.pal(n = 9, name =
  "YlOrRd"))(100), border_color = NA, labels_row = row.labs, fontsize_row = 15, fontsize_col = 15, filename = "~/Figures/fst_heatmap_NoCent_elevation_Dec2022_10k_pop.png",height = 9, width = 9)
