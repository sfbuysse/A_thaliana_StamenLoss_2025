...
Using the PVE plot, I am going to choose the first 4 PCs to use to look for a signal of selection. Emily suggested using the ones that are correlated with elevation so maybe this will change later on but starting with 4 becuase there is a break in the pve after  4.
...
Set to not evaluate while I figure out some coloring stuff. The last population isnot included the y axis. why not? this plotting is kinda confusing to me. and it makes lining things up a bit confusing, but I suppose I can just use the same subsetting that Emily used to remove the last row?
...
ggplot(forplot)+
  geom_point(aes(x = eig_vec1 , y = Pheno, shape = as.factor(Population), fill = Elev_m), col = "black", size = 5, stroke = 1.75)+
  #geom_smooth(aes(x = eig_vec1 , y = Pheno), method = "lm", se = TRUE, level = 0.95, fullrange = TRUE)+
  geom_abline(intercept = m2$coefficients[1], slope = m2$coefficients[2], linetype = "solid", alpha = 0.7, linewidth = 1.25)+
  geom_abline(intercept=mean(stamenTrait$Pheno), slope = stamenCI[1], linetype = "solid",
